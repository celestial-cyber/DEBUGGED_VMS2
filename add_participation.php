...
$events = mysqli_query($conn, "SELECT event_id, event_name FROM tbl_events ORDER BY event_date DESC");
...
if(isset($_POST['sbt-part'])) {
  ...
  if (empty($activity_name) || $participant_count < 0 || $event_id <= 0) {
    ...
    $check_sql = "SELECT * FROM tbl_event_participation WHERE activity_name = '$activity_name' AND event_id = $event_id";
    $result = mysqli_query($conn, $check_sql);
        
    if (mysqli_num_rows($result) > 0) {
      // Update existing activity for this event
      $update_sql = "UPDATE tbl_event_participation SET participant_count = participant_count + $participant_count WHERE activity_name = '$activity_name' AND event_id = $event_id";
      ...
      $insert_sql = "INSERT INTO tbl_event_participation (event_id, activity_name, participant_count)
