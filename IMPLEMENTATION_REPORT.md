...
6. Begins database transaction
   ...
   ├─ Inserts into vms_visitors (synced)
   └─ Commits transaction
   ↓
...
4. Clicks "Login"
   ↓
5. Server queries vms_admin or vms_members
   ↓
6. If valid:
...
2. Query vms_visitors table for self-registered (added_by IS NULL)
3. Verify event_id = 1
...
```
Fix:
1. Verify email/password in vms_admin or vms_members
