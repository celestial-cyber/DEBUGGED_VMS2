...
if(isset($_POST['update_goodies'])){
    ...
    mysqli_query($conn, "DELETE FROM vms_goodies_distribution");

    // Insert new value
    mysqli_query($conn, "INSERT INTO vms_goodies_distribution(quantity, created_at) VALUES ($new_count, NOW())");
...
$member_data = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM vms_members WHERE id='$id'"));

// Dashboard Stats
$total_visitors = mysqli_fetch_row(mysqli_query($conn, "SELECT COUNT(*) FROM vms_visitors"))[0];
$checked_in = mysqli_fetch_row(mysqli_query($conn, "SELECT COUNT(*) FROM vms_visitors WHERE status=1 AND in_time IS NOT NULL"))[0];
$goodies_given = mysqli_fetch_row(mysqli_query($conn, "SELECT COALESCE(SUM(quantity),0) FROM vms_goodies_distribution"))[0];
$event_participants = mysqli_fetch_row(mysqli_query($conn, "SELECT COALESCE(SUM(participant_count),0) FROM vms_event_participation"))[0];

// Fetch recent visitors
$visitors = mysqli_query($conn, "SELECT * FROM vms_visitors ORDER BY created_at DESC LIMIT 10");
...
if(isset($_POST['add_inventory'])){
    ...
    $insert = mysqli_query($conn, "INSERT INTO vms_inventory (item_name, total_stock, used_count, status, created_at)
...
if(isset($_POST['delete_inventory'])){
    $delete = mysqli_query($conn, "DELETE FROM vms_inventory");
    if($delete){
        // Optional: reset auto-increment
        mysqli_query($conn, "ALTER TABLE vms_inventory AUTO_INCREMENT = 1");
...
if(isset($_POST['mark_used'])){
    ...
    $item = mysqli_fetch_assoc(mysqli_query($conn, "SELECT total_stock, used_count FROM vms_inventory WHERE id=$item_id"));
    if($item){
        ...
        mysqli_query($conn, "UPDATE vms_inventory SET used_count=$new_used WHERE id=$item_id");
    }
...
<aside class="sidebar" id="sidebar">
  <div class="brand">
    <img src="../assets/images/SALogo.png" alt="Logo" style="width:50px;height:50px;border-radius:8px;object-fit:cover;margin-left:-15px;">
    <span class="brand-text">SPECANCIENS VMS</span>
  </div>
  <ul class="nav">
    <li><a href="dashboard.php" class="is-active"><i class="fa-solid fa-house me-2"></i>Member Dashboard</a></li>
