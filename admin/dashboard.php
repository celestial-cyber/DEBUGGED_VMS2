...
// ================= Dashboard Stats =================
$total_visitors = mysqli_fetch_row(mysqli_query($conn, "SELECT COUNT(*) FROM vms_visitors"));
$checked_in = mysqli_fetch_row(mysqli_query($conn, "SELECT COUNT(*) FROM vms_visitors WHERE status=1 AND in_time IS NOT NULL"));
$goodies_given = mysqli_fetch_row(mysqli_query($conn, "SELECT COALESCE(SUM(quantity), 0) FROM vms_goodies_distribution"));
$event_participants = mysqli_fetch_row(mysqli_query($conn, "SELECT COALESCE(SUM(participant_count), 0) FROM vms_event_participation"));
...
// ================= Coordinator Notes =================
$notes_log = mysqli_query($conn, "SELECT * FROM vms_coordinator_notes WHERE note_type='LOG' ORDER BY created_at DESC LIMIT 5");
$notes_actions = mysqli_query($conn, "SELECT * FROM vms_coordinator_notes WHERE note_type='ACTION_ITEM' ORDER BY created_at DESC LIMIT 5");
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
  ...
  <li><a href="nostalgia_dashboard.php"><i class="fa-solid fa-scroll"></i><span>Nostalgia</span></a></li>
  ...
  <script>
    ...
    function addParticipation() {
      ...
      fetch('../actions/add_participation.php', {
        method: 'POST',
        body: formData
      })
      .then(response => response.json())
