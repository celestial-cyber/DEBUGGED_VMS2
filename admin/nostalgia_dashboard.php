...
$event_query = mysqli_query($conn, "SELECT event_id FROM vms_events WHERE event_name = '$event_name'");
...
// ================= Dashboard Stats =================
$total_visitors = mysqli_fetch_row(mysqli_query($conn, "SELECT COUNT(*) FROM vms_visitors WHERE event_id = $event_id"));
$checked_in = mysqli_fetch_row(mysqli_query($conn, "SELECT COUNT(*) FROM vms_visitors WHERE event_id = $event_id AND status=1 AND in_time IS NOT NULL"));
$goodies_given = mysqli_fetch_row(mysqli_query($conn, "SELECT SUM(g.quantity) FROM vms_goodies_distribution g JOIN vms_visitors v ON g.visitor_id = v.id WHERE v.event_id = $event_id"));
$event_participants = mysqli_fetch_row(mysqli_query($conn, "SELECT COALESCE(SUM(participant_count),0) FROM vms_event_participation WHERE event_id = $event_id"));

// ================= Visitor Data =================
$visitors = mysqli_query($conn, "SELECT * FROM vms_visitors WHERE event_id = $event_id ORDER BY created_at DESC LIMIT 10");

// ================= Inventory Data =================
$inventory = mysqli_query($conn, "SELECT * FROM vms_inventory ORDER BY status, item_name");

// ================= Goodies Distribution =================
$goodies = mysqli_query($conn, "SELECT g.*, v.name as visitor_name FROM vms_goodies_distribution g LEFT JOIN vms_visitors v ON g.visitor_id = v.id WHERE v.event_id = $event_id ORDER BY g.distribution_time DESC LIMIT 10");

// ================= Event Participation =================
$participation = mysqli_query($conn, "SELECT * FROM vms_event_participation WHERE event_id = $event_id ORDER BY participant_count DESC");

// ================= Coordinator Notes =================
$notes_log = mysqli_query($conn, "SELECT * FROM vms_coordinator_notes WHERE note_type='LOG' AND event_id = $event_id ORDER BY created_at DESC LIMIT 5");
$notes_actions = mysqli_query($conn, "SELECT * FROM vms_coordinator_notes WHERE note_type='ACTION_ITEM' AND event_id = $event_id ORDER BY created_at DESC LIMIT 5");
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
  ...
  <aside class="sidebar" id="sidebar">
    ...
    <ul class="nav">
      <li><a href="dashboard.php"><i class="fa-solid fa-house"></i><span>Admin Dashboard</span></a></li>
      ...
      <div class="col-12 col-lg-7">
        <div class="card-lite">
          <div class="card-head">
            ...
            <div class="d-flex gap-2">
              <button class="btn btn-sm btn-soft" onclick="location.href='../public/new-visitor.php'"><i class="fa-solid fa-plus me-1"></i>Add</button>
              ...
              <table class="table table-sm align-middle">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td><?php echo htmlspecialchars($row['in_time'] ?? 'N/A'); ?></td>
