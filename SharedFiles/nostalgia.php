...
if (isset($_GET['export']) && $_GET['export'] === 'pdf') {
    ...
    $visitors_pdf = mysqli_query($conn, "SELECT * FROM vms_visitors WHERE event_id=$event_id ORDER BY created_at DESC");
...
$event = mysqli_fetch_assoc(mysqli_query($conn, "SELECT event_id FROM vms_events WHERE event_name='" . mysqli_real_escape_string($conn, $nostalgia_name) . "' LIMIT 1"));
if (!$event) {
    mysqli_query($conn, "INSERT INTO vms_events (event_name, event_date) VALUES ('" . mysqli_real_escape_string($conn, $nostalgia_name) . "', NULL)");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        if ($edit_id > 0) {
            $sets = [];
            if (!isGeneratedColumn($conn, 'vms_visitors', 'name')) { $sets[] = "name='$name'"; }
            // also update full_name if exists
            $hasFullName = false;
            $probeFull = mysqli_query($conn, "SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='vms_visitors' AND COLUMN_NAME='full_name' LIMIT 1");
            ...
            $probeMobile = mysqli_query($conn, "SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='vms_visitors' AND COLUMN_NAME='mobile' LIMIT 1");
            ...
            $sql = "UPDATE vms_visitors SET " . implode(',', $sets) . " WHERE id=$edit_id AND event_id=$event_id";
            ...
            if (!isGeneratedColumn($conn, 'vms_visitors', 'name')) { $columns[] = 'name'; $values[] = "'$name'"; }
            // include full_name if present
            $probeFull = mysqli_query($conn, "SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='vms_visitors' AND COLUMN_NAME='full_name' LIMIT 1");
            if ($probeFull && mysqli_num_rows($probeFull) > 0) { $columns[] = 'full_name'; $values[] = "'$name'"; }
            // handle phone/mobile
            $hasMobile = false;
            $probe = mysqli_query($conn, "SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='vms_visitors' AND COLUMN_NAME='mobile' LIMIT 1");
            if ($probe && mysqli_num_rows($probe) > 0) { $hasMobile = true; }
            if ($hasMobile) { $columns[] = 'mobile'; $values[] = "'$phone'"; }
            else { $columns[] = 'phone'; $values[] = "'$phone'"; }
            $sql = "INSERT INTO vms_visitors (".implode(',', $columns).") VALUES (".implode(',', $values).")";
...
if (isset($_GET['action'], $_GET['id'])) {
    ...
    if ($_GET['action'] === 'checkin') {
        mysqli_query($conn, "UPDATE vms_visitors SET in_time=IFNULL(in_time, NOW()), out_time=NULL WHERE id=$vid AND event_id=$event_id");
        $success = 'Checked in.';
    } elseif ($_GET['action'] === 'checkout') {
        mysqli_query($conn, "UPDATE vms_visitors SET out_time=NOW() WHERE id=$vid AND event_id=$event_id AND in_time IS NOT NULL");
        $success = 'Checked out.';
...
if (isset($_GET['action'], $_GET['id']) && $_GET['action'] === 'edit') {
    $eid = (int)$_GET['id'];
    $res = mysqli_query($conn, "SELECT * FROM vms_visitors WHERE id=$eid AND event_id=$event_id LIMIT 1");
    $editing = mysqli_fetch_assoc($res) ?: null;
}

// Load visitors for Nostalgia
$visitors = mysqli_query($conn, "SELECT * FROM vms_visitors WHERE event_id=$event_id ORDER BY created_at DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
