...
ADD CONSTRAINT fk_visitors_added_by FOREIGN KEY (added_by) REFERENCES vms_members(id) ON DELETE SET NULL;
```
...
The `vms_excel_imports` table stores uploaded Excel data before importing to main dashboard:
...
CREATE TABLE IF NOT EXISTS vms_excel_imports (
    ...
    FOREIGN KEY (event_id) REFERENCES vms_events(event_id) ON DELETE CASCADE
);
```
...
2. **Storage**: Data stored in `vms_excel_imports` with status 'pending'
3. **Search**: Use `search-excel.php` to find specific guests
4. **Import**: Click "Import" button to move selected guest to main `vms_visitors` table
...
1. **Spot Entry Registration**
   ...
   - Records in `vms_visitors` with `visitor_type = 'spot_entry'`
...
- Only members in `vms_members` table can access
...
**Query Pattern:**
```php
$event_query = mysqli_query($conn, "SELECT event_id FROM vms_events WHERE event_name = 'Nostalgia'");
...
### Modified Files:
```
/setup.sql                   # Updated vms_visitors schema
...
4. System imports data to vms_excel_imports
...
1. Ensure event exists in `vms_events` table
...
**Solution**:
- Verify event exists in vms_events table
- Check event_id in vms_visitors matches
