...
## Overview
A secure alumni self-registration form has been integrated into the Visitor Management System (VMS). All registrations are automatically saved to both the `alumni_registrations` table and synced to the `tbl_visitors` table for immediate appearance on both admin and member dashboards.
...
2. Corresponding visitor record automatically inserted into `tbl_visitors` with status 1 (In)
