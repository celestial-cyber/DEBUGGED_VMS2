...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_visitor_id'])) {
    ...
    if ($del_id > 0) {
        $dstmt = $conn->prepare("DELETE FROM tbl_visitors WHERE id = ?");
        if ($dstmt) {
                ...
                <table class="table table-sm align-middle">
                    ...
                    <tbody>
                        ...
                        if(isset($_REQUEST['srh-btn']))
                        {
                            ...
                            $countSql = "SELECT COUNT(*) as cnt FROM tbl_visitors v LEFT JOIN tbl_admin a ON v.added_by = a.id LEFT JOIN tbl_members m ON v.added_by = m.id" . $where;
                            $totalRows = 0;
                            ...
                            $sql = "SELECT v.*, COALESCE(a.user_name, m.member_name) AS added_by_name FROM tbl_visitors v LEFT JOIN tbl_admin a ON v.added_by = a.id LEFT JOIN tbl_members m ON v.added_by = m.id" . $where . " ORDER BY v.roll_number ASC, v.created_at DESC LIMIT $offset,$perPage";
                            ...
                            $countRes = mysqli_query($conn, "SELECT COUNT(*) as cnt FROM tbl_visitors");
                            $totalRows = 0;
                            if ($countRes) {
                            ...
                            $select_query = mysqli_query($conn, "SELECT v.*, COALESCE(a.user_name, m.member_name) AS added_by_name FROM tbl_visitors v LEFT JOIN tbl_admin a ON v.added_by = a.id LEFT JOIN tbl_members m ON v.added_by = m.id ORDER BY v.roll_number ASC, v.created_at DESC LIMIT $offset,$perPage");
                            $sn = $offset + 1;
